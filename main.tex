...
\subsection{Separazione tramite Max-Flow / Min-Cut}

Il problema di separazione consiste nell’individuare, a partire da una soluzione frazionaria \(\{x^*_{ij}\}\), se esiste un sottoinsieme \(S\) che viola un vincolo SEC.
Interpretando i valori \(x^*_{ij}\) come capacità degli archi, si calcola per ogni coppia di nodi un taglio minimo; se per qualche taglio la capacità risulta minore di 1, il corrispondente insieme \(S\) definisce un vincolo SEC violato da aggiungere al modello.
...
Nel secondo step il rilassamento viene risolto più volte: a ogni iterazione la soluzione frazionaria viene interpretata come un grafo con capacità pari a $x^*_{ij}$ e si applica una procedura di separazione basata su taglio minimo (Min-Cut). Per ciascun nodo $k\neq 0$ si calcola un taglio minimo che separa $k$ dal deposito; se la capacità del taglio è minore di 1, l'insieme $S$ sul lato della sorgente individua un vincolo SEC violato del tipo $\sum_{i\in S,\,j\notin S} x_{ij}\ge 1$, che viene aggiunto come \textit{user cut}. Il processo prosegue fino a quando non vengono trovate ulteriori violazioni o fino a un numero massimo di iterazioni.

\subsubsection*{Soluzione intera (Step 3)}

Nello step finale viene risolto un modello intero con variabili binarie $x_{ij}$. Quando il solver trova una soluzione intera che contiene più cicli disgiunti, tali sottotour vengono individuati tramite componenti fortemente connesse sul grafo degli archi selezionati e i corrispondenti vincoli SEC vengono aggiunti come \textit{lazy constraints}. In questo modo si ottiene un vero algoritmo Branch-and-Cut.
Al termine si ottiene un tour hamiltoniano unico: il codice ricostruisce la sequenza ordinata degli archi a partire dal deposito, calcola la distanza totale in metri (\texttt{step3\_integer\_solution}) e il tempo complessivo di percorrenza sommando le durate su ciascun arco (\texttt{final\_time}).
...
Nel rilassamento si introducono variabili di posizionamento \(u_i\in[0,n-1]\) e vincoli \(u_i - u_j + n x_{ij} \le n-1\) per tutti i nodi diversi dal deposito, ottenendo un bound continuo alternativo rispetto alla formulazione a tagli. 
...
Nel caso con 50 nodi il bound continuo MTZ risulta di circa \(100\,089\) m, mentre la soluzione intera richiede l’intero time limit per raggiungere il valore dell’ottimo trovato con il Cut-Set.
